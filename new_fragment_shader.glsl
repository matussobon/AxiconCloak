...
struct IdealThinLensSurface {
	vec3 principalPoint;
    vec3 opticalAxisDirection;
    float focalLength;
    float transmissionCoefficient;
};
uniform IdealThinLensSurface idealThinLensSurfaces[ 2 ];
...
bool findNearestIntersectionWithObjects(
	...
	while( i < rectangles.length() ) {
		...
		if((startObjectType != 0) || (startObjectIndex != i)) {
			...
			if(intersectionDistance < closestIntersectionDistance) {
				...
				closestIntersectionObjectType = 0;	// rectangle
...
vec3 lensDeflect(vec3 d, vec3 p2i, vec3 axisHat, float f, bool idealLens) {
    ...
    if(idealLens) {
        ...
        // d' \propto d - d_axial / f (I - P)

        return d - (dot(d, axisHat)/f)*p2i;
	}
}

bool findNearestIntersectionWithObjectsTest(
	vec3 s, 
	vec3 d,
	in int startObjectType,	// rectangle = 0
	in int startObjectIndex,
	...
	// check if the ray started on this rectangle
		//if((startObjectType != 0) || (startObjectIndex != i)) {
			...
			if (findNearestIntersectionWithRectangle(
				s, d, rectangles[i].corner, rectangles[i].uSpanVector,rectangles[i].vSpanVector, 
				1., 1., intersectionPosition, intersectionDistance, intersectionNormal
			)) {
			if(intersectionDistance < closestIntersectionDistance) {
				closestIntersectionPosition = intersectionPosition;
				closestIntersectionDistance = intersectionDistance;
				closestIntersectionNormal = intersectionNormal;
				closestIntersectionObjectType = 0;	// rectangle
				closestIntersectionObjectIndex = 1;
				closestIntersectionSurfaceType = rectangles[0].surfaceType;
				closestIntersectionSurfaceIndex = rectangles[0].surfaceIndex;
			}
		//}
		i++;
			}
...
void main() {
	...
	for(int i=0; i<noOfRays; i++) {
		...
		while(
			...
			if(intersectionObjectType == 0) { 
				 //color = vec4(1., .5, .5, 1.);
				 LightRay.direction = lensDeflect(LightRay.direction, intersectionPosition-idealThinLensSurfaces[0].principalPoint, 
				 									idealThinLensSurfaces[0].opticalAxisDirection, idealThinLensSurfaces[0].focalLength, 
													true);
				 //tl=-10;
			}
